...
export default class SiderMenu extends PureComponent {
  ...
  getDefaultCollapsedSubMenus(props) {
    ...
    return urlToList(pathname)
      .map(item => {
        ...
        return getMeunMatchKeys(this.getFlatMenuKeys(props.menuData), item)[0];
      })
      .filter(item => item);
  }
  /**
  ...
  getSelectedMenuKeys = () => {
    ...
    return urlToList(pathname).map(itemPath =>
      getMeunMatchKeys(
        this.getFlatMenuKeys(this.props.menuData),
        itemPath
      ).pop()
    );
  };
  ...
  isMainMenu = key => {
    ...
    return this.props.menuData.some(
  ...
  render() {
    const { logo, collapsed, onCollapse, menuData, gettingCommon } = this.props;
    ...
    return (
      ...
      >
        ...
        <Spin size="large" spinning={gettingCommon}>
          <Menu
            key="Menu"
            theme="light"
            mode="inline"
